...
shinyServer(function(input, output, session) {
  ...
  output$ScatterPlot <- renderPlot({
    ...
    req(input$plotType == "scatter", input$x, input$y)
    x_col <- "rainfall"
    y_col <- "temperature"
    z_col <- "Crop" 
    plot_ly(data, x = ~get(x_col), y = ~get(y_col), z = ~get(z_col), type = "scatter3d", mode = "markers") %>%
      layout(scene = list(xaxis = list(title = x_col), yaxis = list(title = y_col), zaxis = list(title = z_col)))
